...
export const findReduxFormField = (wrapper, prop, value) => {
  return wrapper.find(Field).findWhere(el => el.props()[prop] === value);
}

export const findButtonWithLabel = (wrapper, label) => {
  return wrapper.find(Button).filterWhere(el => el.props().children === label).first();
}